...
Most of the techniques discussed as part of these tutorial use **holistic network analysis**. Holistic network data refer to a complete set of relationships that exist as part of a bounded collective unit. By "bounded," I mean that the boundaries for the group are clear. For example, in a classroom a student either is or is not on the roster. A researcher can identify those boundaries and then survey these students about who they study with. These responses can then be converted to an adjacency matrix showing relationships between all of these students.

Let's initialize the necessary packages...
...
...and then we can generate a familiar example of a holistic network random graph, perhaps of friendships between students in a class.
...
In many social networks, though, boundaries are either unclearly defined, inaccessible, or too large to study collectively. Let's say we want to know about the relationships among people who are fans of Manga graphic novels.
...
So instead of one big network defining all of the relationships within the group, we obtain multiple smaller networks that revolve around the relationships of a single focal person, **ego** (which is why these data are referred to as "egocentric").

Let's examine an example of what an ego network might look like. We'll start by assigning values to the number of **alters** in the network, a case identifier for ego, and case identifiers for each alter (2,3,4,5,6).
...
With these data, we can estimate a set of ties between ego and the alters. The first line below creates an edgelist that includes ties between ego and all alters. (This is a requirement. An alter has to be connected to ego in order to be in that ego's network.) Then we sample all possible combinations of ties between alters, binding those random alter edges with the ego ties in order to complete the ego network edgelist.
...
Finally, we create an `igraph` object and plot the graph.
...
We will demonstrate egocentric network analysis using data from the General Social Survey. This demonstration is an adaptation from an excellent textbook from Raffaele Vacca devoted to ego network analysis, which we highly recommend to those who are interested in more advanced coding on this topic.[^egonet_setup-1]

[^egonet_setup-1]: Vacca, Raffaele. 2024. Egocentric network analysis with R. <https://raffaelevacca.github.io/egocentric-r-book/>
...
The GSS is a national probability sample of adults in the United States that has been conducted annually or biannually every year since the 1970s. It is a valuable source of information about the attitudes and behaviors of the US population.

We will work with the 2004 dataset because that year included a detailed module of questions asking people to identify the people with whom they discuss important matters, which I will refer to as "confidants."[^egonet_setup-2] The responses to this initial question are available in the "NUMGIVEN" variable.

[^egonet_setup-2]: McPherson, Miller, Lynn Smith-Lovin, and Matthew E. Brashears. 2006. “Social Isolation in America: Changes in Core Discussion Networks over Two Decades.” American Sociological Review 71(3):353–75.
...
Much has been written about what this might mean, either in terms of how friendships are changing over time or whether these estimates are methodological artifacts.[^egonet_setup-3] [^egonet_setup-4] [^egonet_setup-5] However, we are going to sidestep these debates and instead proceed with analysis of ego networks with two or more confidants.

[^egonet_setup-3]: Bearman, Peter, and Paolo Parigi. 2004. “Cloning Headless Frogs and Other Important Matters: Conversation Topics and Network Structure.” Social Forces 83(2):535–57.

[^egonet_setup-4]: Fischer, Claude S. 2009. “The 2004 GSS Finding of Shrunken Social Networks: An Artifact?” American Sociological Review 74(4):657–69.

[^egonet_setup-5]: Small, Mario Luis. 2017. Someone to Talk To. Oxford University Press.
...
This new alterlong data frame has exactly five times the number of observations as the original alter file (288\*5 = 1440). That's because each ego now has five lines, given the potential for having up to five different alters. "alterlong" also has much fewer variables, because each of the attributes that used to have five different variables (e.g., PARENT1-5) now has only a single variable (PARENT).
...
```{r}
alterlong <- alterlong |> 
  filter(alter_id <= NUMGIVEN) |> # keep only rows where ALTERID <= NUMGIVEN
  select(-NUMGIVEN) # remove the NUMGIVEN variable
...
Now we can see that ego #10's fourth alter has an alter_id of 104. This will be very helpful when we visualize these networks later.
...
These variables measure how close ego thinks that the two alters are to one another. We will examine the specific values later. For now, just note that the numbers indicate which alters are being referenced. For example, CLOSE25 refers to the perceived closeness between alter 2 and alter 5.

To create the edgelist, we need to `pivot_long()` so that each line represents a unique relationship between alters.
...
Now let's take a closer look at the first ego network (without ego, alters only). To do this, we can subset the "gr.list" with a double bracket to select the first item in the list. Then let's look at the vertex sequence.
...
Now that the ego network data have been properly formatted, we can save the objects as an R readable ".rda" file. I've commented this out because the data have already been saved. We will load these objects and examine them in the next chapter.
