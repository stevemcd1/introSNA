...
Once the egocentric network data files have been constructed, we can turn to analyzing the data. As a reminder, this demonstration is an adaptation from Raffaele Vacca's highly recommended textbook devoted to ego network analysis.[^egonet_analysis-1]

[^egonet_analysis-1]: Vacca, Raffaele. 2024. Egocentric network analysis with R. <https://raffaelevacca.github.io/egocentric-r-book/>

First we will add needed ego attributes to the ego list files, then we will show basic visualization strategies, and then we will examine metrics across ego, alter, and edge level characteristics.
...
Let's begin by examining a single ego network. Below we store new `igraph` objects -- one without ego and one with ego included -- for the ego network number 10 in our larger list.
...
One final representation to show is communities, which relies on algorithms to detect optimal groups based on modularity (if this is unfamiliar to you, check out the chapter on community detection). Below we use the Louvain algorithm to identify modular groups. But this time, we will present the network for ego number 80.
...
For the purpose of analysis, it will be necessary to obtain these estimates for all egos. This will require the use of `map_dbl()`. Below we demonstrate how this is achieved for a single metric (edge_density).
...
Next we generate a single row of network metrics for a single ego network.
...
Let's bring this all together by estimating these edge characteristics for all of the ego networks and combining them into a data frame.
...
With a data set like this, it is possible to conduct various statistical analyses. Below we show a simple analysis of variance (**ANOVA**) test to demonstrate one possibility.
...
These are simple bivariate analyses. More complex analyses based on multivariate and multilevel analyses are also possible with egocentric network data using multilevel regression modeling (see <https://raffaelevacca.github.io/egocentric-r-book/multilevel.html>).

This chapter and the previous one presented a unique form of social network analysis that focuses on the personal relationships that surround individuals. This is a very useful approach to take especially when sampling large groups of people and in contexts that lack clear group boundaries.[^egonet_analysis-2]

[^egonet_analysis-2]: McCarty, C., Lubbers, M. J., Vacca, R., & Molina, J. L. 2019. Conducting personal network research: A practical guide. The Guilford Press.
