...
One of our favorite TV shows is "Flight of the Conchords," which was a sitcom about a New Zealand folk-rock band living in New York City. Below I will generate a network graph from the friendships shown in [season 2 episode 4](https://www.imdb.com/title/tt1313336/?ref_=tt_epspo_ep_4), "Murray Takes it to the Next Level."[^networks-1] Here's a picture of the guys from that episode.

[^networks-1]: "Murray Takes It to the Next Level." *Flight of the Conchords*, season 2, episode 4, written by James Bobin, Jemaine Clement, and Bret McKenzie, directed by Troy Miller, HBO, February 8, 2009.
...
Now let's examine the elements of the object we created for the undirected conchords network.
...
Hmm...this does not show the new values, only the binary ones. We have to extract the matrix using the `as_adjacency_matrix` function and include an option to apply the weighted values.
...
Here you can see that the conchord object was effectively duplicated.
...
Using these two data frames, we can create a new `igraph` object using the 'graph_from_data_frame' function. We first need to assign the edge_df as "d", then indicate that those edges are undirected, and finally add the vertex attributes.
