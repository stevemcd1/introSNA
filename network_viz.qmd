...
Now let's load and examine the high tech managers data. These data were collected by David Krackhardt in the mid-1980s by surveying the managers of a small technology firm in California.[^network_viz-1] (The CEO of the company is also included as one of the managers.) This dataset is especially good to illustrate network visualization because it is small and contains many vertex attributes.

[^network_viz-1]: Krackhardt D. (1987). Cognitive social structures. Social Networks, 9, 104-134.
...
Let's add other node attributes to the graph. We can base the shapes on the supervisory levels and the size based on their tenure. And now that we are displaying multiple attributes, the legend cab be added back in.
